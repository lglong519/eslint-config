...
module.exports = {
	...
	'rules': {
		...
		'curly': [
			0,
			'multi'
		], // [f][m] 多行条件语句必需大括号，enforce consistent brace style for all control statements
		'default-case': 0, // case 必需 default 分支
		'dot-location': [
			2,
			'property'
		], // [f]
		...
		'no-alert': 0, //
		'no-caller': 0, //
		'no-case-declarations': 0, // [r][m]2-0 不允许在 case 子句中使用词法声明
		...
		'strict': [
			2,
			'safe'// 如果 eslint 认为文件是 CommonJS，启动 global 模式：要求在全局作用域下有且只有一个严格模式指令
		], // 认为文件不是 CommonJS，启动 function 模式：函数声明或表达式开始位置有一个严格模式指令
		// *** Variables
		'init-declarations': 0, //
		...
		'no-undefined': 0, // [m] 允许将 undefined 作为标识符
		'no-unused-vars': 1, // [r][m]
		...
		'func-call-spacing': 2, // [f] 函数名与调用括号间的空格
